...
正式复习的视觉区域已进一步拆分至 `formal_review_widgets.dart`：沉浸式背景与响应式布局、顶部进度/操作栏、单词提示、四选一候选卡片、底部答案动作、加载、失败和完成视图均成为独立组件。`ReviewPage` 现在只保留路由生命周期、会话与词条操作状态组合、收藏/掌握反馈、音频和详情导航等页面协调职责，避免再次演变为包含业务状态与大量视觉细节的超长文件。
