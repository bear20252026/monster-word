...
| **D 架构违规批 ✅（2026-09-04 完成）** | ①3 处直连收口：forceRebuild/diagnostics 收进新服务 `core/application/wordbook_maintenance_service.dart`（book/settings providers 注入，类型 export 转发），word_detail 的 getWord 改走既有 WordRepository Provider 通道（getWordByText 语义等价：同为 WHERE word=? limit 1）；②ImportGuard 新增 R-DB 规则（presentation 禁 import wordbook_database/user_database）+ import_guard_test 判定用例，正向直连从此 CI 拦得住（REG-ARCH-005） | 架构收口 | 中（已完成） |
| **E FSRS 迁 SQLite 专项 ✅（2026-09-04 完成，E1）** | 方案经用户批准（三拍板项全部通过）后落地：新增 `lib/features/learning/data/review_schedule_store.dart`（独立 review_schedule.db：fsrs_cards/fsrs_daily_stats/fsrs_active_dates 三表，与词库物理隔离防重建误伤）；写路径从每次评分 3 次全量 jsonEncode 重写改为单事务 3 语句 O(1)；首启迁移：SQLite 空 + SP 非空 → 逐行解析（损坏行跳过 debugPrint + 聚合 Sentry 上报）→ 事务导入（行数校验在提交前，不达标整体回滚）→ 全部成功才写 `fsrs6_migrated_v1` 标记；任何失败 → 本机降级 SP 模式零损失 + 下次启动重试；旧 SP key 保留为只读回滚快照（E2 清理观察 2~3 版后另批执行）；明确不做双写。测试：store 4 用例 + 迁移 6 用例（sqflite_common_ffi 内存库，CI 先例） | 数据迁移 | 高（已完成） |
