...
class ShareImageService {
  ...
  static Future<Uint8List> generateShareImage({
    ...
    canvas.drawLine(
      ...
      Offset(width * 0.9, height * 0.25),
      linePaint,
    );

    // === 标题文字 ===
    _drawText(canvas, 'Monster Word', width / 2, height * 0.12,
      fontSize: 72,
      color: Colors.white,
      fontWeight: FontWeight.bold,
    );

    _drawText(canvas, '背单词 · 有态度', width / 2, height * 0.18,
      fontSize: 36,
      color: const Color(0xFFCBA258),
    );
    ...
    _drawStatCard(canvas, width * 0.1, cardTop, width * 0.8, cardHeight,
      '累计学习', '$totalDays 天', Icons.calendar_today);
    _drawStatCard(canvas, width * 0.1, cardTop + cardHeight + 30, width * 0.8, cardHeight,
      '掌握单词', '$totalWords 个', Icons.menu_book);
    _drawStatCard(canvas, width * 0.1, cardTop + (cardHeight + 30) * 2, width * 0.8, cardHeight,
      '连续签到', '$streakDays 天 🔥', Icons.local_fire_department);

    // === 底部装饰 ===
    final bottomY = height * 0.88;
    _drawText(canvas, '每一天，都在进步', width / 2, bottomY,
      fontSize: 32,
      color: Colors.white.withValues(alpha: 0.7),
    );

    _drawText(canvas, '— Monster Word —', width / 2, bottomY + 60,
      fontSize: 24,
      color: const Color(0xFFCBA258),
    );
    ...
    _drawText(canvas, '扫码下载 Monster Word', width / 2, qrTop + qrSize + 30,
      fontSize: 20,
  ...
  static void _drawStatCard(
    ...
    final cardPaint = Paint()
      ...
      ..style = PaintingStyle.fill;
    canvas.drawRRect(
      RRect.fromRectAndRadius(Rect.fromLTWH(left, top, w, h), const Radius.circular(20)),
      cardPaint,
    );
    ...
    // 图标
    _drawText(canvas, String.fromCharCode(icon.codePoint), left + 60, top + h / 2 - 20,
      fontSize: 48, color: const Color(0xFFCBA258));

    // 标签
    _drawText(canvas, label, left + 130, top + h / 2 - 30,
      fontSize: 28, color: Colors.white.withValues(alpha: 0.7));

    // 数值
    _drawText(canvas, value, left + 130, top + h / 2 + 30,
  ...
  static Future<String> generateAndSave({
    ...
    final bytes = await generateShareImage(
  ...
  static Future<void> generateAndShare({
    ...
    final filePath = await generateAndSave(
      totalWords: totalWords,
      streakDays: streakDays,
      totalDays: totalDays,
    );
