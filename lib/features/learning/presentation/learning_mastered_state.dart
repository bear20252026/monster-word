...
/// 手动掌握标记的读取与操作状态。
///
/// 持久化继续委托 [MasteredRepository]，以保持 `mastered_words_v1` 的字符串身份和
/// 已有用户数据兼容。该状态仅提供页面可订阅的掌握词集合与切换结果。
class LearningMasteredState extends ChangeNotifier {
  LearningMasteredState({required MasteredRepository masteredRepository}) : _masteredRepository = masteredRepository {
    unawaited(refresh());
  }
  ...
  Future<void> refresh() async {
    ...
    notifyListeners();
    try {
      _masteredWords = (await _masteredRepository.getMasteredWords()).toSet();
    } finally {
      _isLoading = false;
      notifyListeners();
    }
