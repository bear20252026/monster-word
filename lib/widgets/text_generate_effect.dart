...
class TextGenerateEffectMultiLine extends StatefulWidget {
  ...
  @override
  State<TextGenerateEffectMultiLine> createState() => _TextGenerateEffectMultiLineState();
}

class _TextGenerateEffectMultiLineState extends State<TextGenerateEffectMultiLine> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    for (int i = 0; i < widget.lines.length; i++) {
      final controller = AnimationController(
        vsync: this,
        duration: Duration(milliseconds: widget.lines[i].length * widget.charDuration.inMilliseconds),
      );
      _controllers.add(controller);
      _characterCounts.add(
        StepTween(
          begin: 0,
          end: widget.lines[i].length,
    ...
    for (int i = 0; i < _controllers.length; i++) {
      Future.delayed(Duration(milliseconds: i * widget.lineDelay.inMilliseconds), () {
        if (mounted) {
