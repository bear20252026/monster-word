...
class StudyRemindText {
  ...
  StudyRemindText({this.notifyType = 0, this.type = 0, this.content = '', this.startTime = 0, this.endTime = 0});

  factory StudyRemindText.fromJson(Map<String, dynamic> json) => StudyRemindText(
...
class StudyRemindTextConfig {
  ...
  StudyRemindTextConfig({List<StudyRemindText>? datas}) : datas = datas ?? [];
  ...
  String getRemindText(int type, int timestampMs, int reviewCount) {
    ...
    switch (type) {
      ...
      case 1:
        final text = _getRemindTextFromList(_noReviewNoOpenedList, timestampMs, 1);
        return text.isNotEmpty ? text : '别着急，单词就是每天一个一个一个一个一个一个一个背完的，今日打卡>>';
      case 2:
        final text = _getRemindTextFromList(_noReviewOpendList, timestampMs, 1);
  ...
  String _getRemindTextFromList(List<StudyRemindText> list, int timestampMs, int notifyType) {
    final ts = timestampMs ~/ 1000;

    // 先检查日期列表
    for (final item in _dateList) {
      if (item.notifyType == notifyType && ts > item.startTime && ts < item.endTime) {
        return item.content;
      }
    }
  ...
  factory StudyRemindTextConfig.fromJson(Map<String, dynamic> json) => StudyRemindTextConfig(
    datas: (json['datas'] as List?)?.map((e) => StudyRemindText.fromJson(e as Map<String, dynamic>)).toList() ?? [],
  );
}
