...
void main() {
  ...
  group('Book list', () {
    ...
    test('Book count within curated range (40-60)', () async {
      ...
      expect(allBooks.length, greaterThanOrEqualTo(40),
          reason: 'Curated library should have at least 40 books, got ${allBooks.length}');
      expect(allBooks.length, lessThanOrEqualTo(60),
  ...
  group('Words by book', () {
    ...
    test('All sampled books have words', () async {
      final samples = books.take(5).toList()
        ..addAll(books.where((b) => b.code.toUpperCase().contains('CET4')).take(2));
    ...
    test('Word field coverage meets real-world standard', () async {
      final cet4Book = books.firstWhere(
          (b) => b.code.toUpperCase().contains('CET4'),
          orElse: () => books.first);
      final words =
          await WordBookDatabase.instance.getWordsByBook(cet4Book.id);
      expect(words.length, greaterThan(10),
          reason: 'Book ${cet4Book.name} should have plenty of words');
      ...
      expect(noInterpret * 100 <= words.length * 50, true,
          reason: 'Interpret gap rate abnormal: $noInterpret/${words.length} (book=${cet4Book.code})');
      expect(noPhonetic * 100 <= words.length * 50, true,
    ...
    test('getWord round-trip consistency', () async {
      final cet4Book = books.firstWhere(
          ...
          orElse: () => books.first);
      final words =
          await WordBookDatabase.instance.getWordsByBook(cet4Book.id, limit: 50);
      final probe = words.firstWhere((w) => w.interpret.trim().isNotEmpty,
          orElse: () => words.first);
      ...
      if (probe.interpret.trim().isNotEmpty) {
        ...
        expect(got.ukPron.isNotEmpty || got.usPron.isNotEmpty, true,
  ...
  group('Search', () {
    test('Prefix search available', () async {
      ...
      final any =
          await WordBookDatabase.instance.getWordsByBook(books.first.id, limit: 1);
